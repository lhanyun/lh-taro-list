...
export default function createListComponent ({
  ...
  return _temp = _class = class List extends PureComponent {
    ...
    // 当滑动到边界值时，没有渲染出足够的item，
    resetScroll = (type) => {
      if (this._observerTimeoutId !== null) {
        cancelTimeout(this._observerTimeoutId)
      }

      this._observerTimeoutId = requestTimeout(() => {
        this._observerTimeoutId = null

          const { scrollOffset } = this.state
          const { height, itemData } = this.props
         
          const totalHeight = this._getCountSize(this.props, itemData.length)
         
          if (scrollOffset > 1 && type === 'top') {
            this.scrollToItem(0)
          } else if (scrollOffset < totalHeight - height && type === 'bottom') {
            // this.scrollToItem(itemData.length - 1)
          }
    ...
    renderObserverView() {
      const { observerPosition, itemCount, observerView } = this.props;
    
      return createElement(observerView, {
    ...
    render () {
      ...
      const items = [this.renderObserverView()]
      ...
      if (itemCount > 0) {
        for (let index = startIndex; index <= stopIndex; index++) {
          ...
          let newKey = key
          if (this.props.customSize) {
            newKey = `${key}-${style.top}-${style.height}`
          }
